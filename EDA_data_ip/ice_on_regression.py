...
df = df.dropna(subset=['ice_on'])

df['ice_on_day'] = df['ice_on'].dt.dayofyear
df['ice_on_year'] = df['ice_on'].dt.year

df.loc[df['ice_on_year'] > df['year'] - 1, 'ice_on_day'] += 365

yearly_avg = df.groupby('year')['ice_on_day'].mean().reset_index()

x = yearly_avg['year'].values
y = yearly_avg['ice_on_day'].values
...
plt.plot(x, y, marker='o', linestyle='-', color='b', label="Yearly Avg Ice On Day")
...
plt.ylabel("Average Ice On Day (Day of Year)", fontsize=18, labelpad=10)
plt.title("Trend of Ice On Day Over Time with Confidence Interval", fontsize=22, pad=15)
plt.legend()
plt.grid(True)
plt.show()
