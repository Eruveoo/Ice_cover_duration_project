...
df['ice_duration'] = (df['ice_off'] - df['ice_on']).dt.days

yearly_avg = df.groupby('year')['ice_duration'].mean().reset_index()

x = yearly_avg['year'].values
y = yearly_avg['ice_duration'].values
...
plt.plot(x, y, marker='o', linestyle='-', color='b', label="Yearly Avg Ice Duration")
...
plt.ylabel("Average Ice Cover Duration (Days)", fontsize=18, labelpad=10)
plt.title("Trend of Average Ice Cover Duration Over Time with Confidence Interval", fontsize=22, pad=15)
plt.legend()
plt.grid(True)
plt.show()
